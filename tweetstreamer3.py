...
class CustomStreamListener(tweepy.StreamListener):
    ...
    def on_status(self, status):
        ...
        if "rt" not in tweet:
            print(color.BOLD + "*",sentiment,"",status.author.screen_name, "", status.created_at, "", tweet+ color.END)
        else:
            print(sentiment,"",status.author.screen_name, "", status.created_at, "", tweet)
        ...
        with open(filename, 'a', encoding='utf8') as f:
            if "RT" not in tweet:
                writer = csv.writer(f)
                writer.writerow([status.author.screen_name, status.created_at, status.text.encode('utf-8'),'0',sentiment])
            else:
                writer = csv.writer(f)
                writer.writerow([status.author.screen_name, status.created_at, status.text.encode('utf-8'), '1',sentiment])
            f.close()
...
def write_titles(filename):
    with open(filename, 'a') as f:
            writer = csv.writer(f)
            writer.writerow(['Author', 'Date', 'Text','RT','Sentiment'])
            f.close()
